...
- **Deleting a data source silently did nothing**: readings and aggregates hold a non-nullable FK with `DeleteBehavior.Restrict`, so any source that had ever recorded a reading could not be removed — SQL Server rejected the delete and the 500 was thrown away by a client that reloaded regardless, making failure and success look identical. The endpoint now counts its dependents first and answers **409** with the real numbers; confirming re-sends with `force=true`, which deletes the source's readings and aggregates in 50,000-row batches and the source itself in one transaction.
- **Admin actions that failed said nothing**: seven actions across five pages were `await fetch(…); location.reload();` with no check on the response, so a rejected delete or a failed toggle reloaded the page and showed the unchanged row. All of them now go through one shared `adminFetch` helper that surfaces the server's message and reloads only when the action actually worked.
...
- **Per-device channel overrides**: a device can redirect its readings to different channels than the ones its data source's `fieldMapping` names — `temperature` → `temperature.outdoor` for an outdoor sensor sharing a source with an indoor one. `fieldMapping` is per source, so identical boards flashed from one sketch all published the same field names and landed on the same channels; renaming the device changed only its display name, leaving an outdoor sensor reporting `temperature.indoor@outdoor-01` with indoor sanity bounds and feeding `dewpoint.indoor`. Overrides are applied before validation, so bounds, calibration offsets and calculated channels all follow the channel the reading ends up on. Edited under **Admin → Devices → Edit**; the key matches either the payload field name or the channel the source resolved it to.
...
- **Device-scoped tiles showed a live value but no high/low**: `GET /api/weather/daily-extremes` keyed its response by bare channel name only, so a widget bound to `temperature.indoor@indoor-01` looked itself up by that key and found nothing. Device-scoped keys are now emitted alongside the bare ones.
- **Today's high/low pooled every device under the bare channel name**: with two sensors on the same channel, a tile took its live reading from the primary device but its high/low from whichever device happened to be warmest — an attic sensor set the day's high for the living-room widget. Bare keys now cover only sourceless readings plus the primary device's, exactly the set `GetCurrentReadingsAsync` publishes under those keys.
- **Widget companion channels ignored the bound device**: a temperature or humidity widget pointed at one device's channel paired it with the station-wide humidity and dew point, mixing two rooms in one tile. Companion channels now follow the device of the channel they accompany, falling back to the station-wide reading for devices that report only some of a widget's channels.
- **Stale badge never appeared on widgets bound to the primary device's channels**: `staleChannels` reported the device-scoped key only, so a widget still bound to `temperature.indoor` kept displaying a last-known value with no warning after the sensor stopped reporting. The primary device's stale channels are now reported under both keys, matching how its readings are published.
- **Feels Like widget ignored its channel picker**: the widget editor offers a channel selector for it and saved the choice, but the renderer had `feelslike.outdoor` hardcoded. It now honors `config.channel`, which matters since feels-like is published per device as well as station-wide.
